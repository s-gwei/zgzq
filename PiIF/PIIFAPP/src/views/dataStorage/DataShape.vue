    ...
    <a-card :bordered="false" style="border-bottom: 1px solid #f1f1f1;">
      <a-row :gutter="48" type="flex" justify="start" style="margin-bottom: 20px;">
        <a-col :style="{ borderRight: '1px solid #f1f1f1'}">
          <span :style="{fontSize: '24px', lineHeight: '40px', whiteSpace: 'nowrap'}">数据形状</span>
        </a-col>
        <a-col >
          <a-form layout="inline">
            <a-form-item label="">
              <a-input-search v-model="queryParam.name" @search="editData" placeholder="输入数据形状名称查询"/>
            </a-form-item>
                ...
                <template slot="content">
                  <a-form style="width:400px;margin-top: 20px;">
                    <a-form-item label="标签:" :labelCol="labelCol" :wrapperCol="wrapperCol">
                      <a-select v-model="queryParam.tags" placeholder="请选择标签" :default-value="0">
...
<!--                    </a-form-item>-->
                    <a-form-item label="项目" :labelCol="labelCol" :wrapperCol="wrapperCol">
                      <a-select  v-model="queryParam.projectName" placeholder="请选择项目">
  ...
  export default {
    ...
    methods: {
      ...
      goDetail (num, record) {
        switch (num) {
          case 1:      // 添加逻辑
            this.$router.push({path: '/dataStorage/ShapeDetail'})
            sessionStorage.setItem("dataShapeInfo",JSON.stringify({tabKey: 1}));
            this.$bus.$emit('setCollapsed', true)
            break;
          case 2:      // 查看逻辑
            this.$router.push({path: '/dataStorage/ShapeDetail'})
            sessionStorage.setItem("dataShapeInfo",JSON.stringify({
              tabKey: 1,
              name: record.name,
