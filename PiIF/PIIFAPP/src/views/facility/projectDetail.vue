  ...
  <a-card :bordered="false">
    ...
    <a-layout style="padding: 24px 0 24px -24px; background: #fff">
      <a-layout-sider width="150" style="background: #fff">
        ...
        >
          <a-sub-menu key="sub1">
            <span slot="title" ><a-icon style="font-size: 16px;" type="inbox" /><span>项目</span></span>
...
<script>
  import Detail from '@views/facility/project/Detail'
  export default {
    name: 'projectDetail',
    data () {
      return {
    ...
    methods: {
      ...
      getThingName(data, id){
        ...
        sessionStorage.setItem('projectInfo', JSON.stringify(this.ModelInfo))
      },
      goback () {
        this.$router.go(-1)
      }
