    ...
    <a-card :bordered="false" style="border-bottom: 1px solid #f1f1f1;">
      <a-row :gutter="48" type="flex" justify="start" style="margin-bottom: 20px;">
        <a-col :style="{ borderRight: '1px solid #f1f1f1'}">
          <span :style="{fontSize: '24px', lineHeight: '40px', whiteSpace: 'nowrap'}">数据流</span>
        </a-col>
        <a-col >
          <a-form layout="inline">
            <a-form-item label="">
              <a-input-search v-model="queryParam.name" @search="editData" placeholder="输入数据流名称查询"/>
            </a-form-item>
      ...
      goDetail (num, record) {
        switch (num) {
          case 1:      // 添加逻辑
            ...
            sessionStorage.setItem("StreamInfo",JSON.stringify({tabKey: 1,type:'dataStream'}));
