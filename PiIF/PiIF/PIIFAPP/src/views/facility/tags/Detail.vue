...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getTagsValue () {
        if (this.tagsId) {
          getAction(this.url.file, {id: this.tagsId}).then(
            (res) => {
              if (res.success) {
                this.editTag = res.result
                this.$nextTick(() => {
                  this.form.setFieldsValue(pick(this.editTag, 'name','desciption', 'document', 'mashupName', 'objectPk', 'projectPk', 'tagPk'));
                  this.img = this.editTag.icon
                })
              } else {
                this.$message.error(res.message);
              }
           ...
           console.log(values)
            if (!this.tagsId) {
              ...
              let params = {
                ...
                mashupName: values.mashupName  ? values.mashupName : '',
                name: values.name  ? values.name : '',
                objectPk: this.editTag.objectPk ? this.editTag.objectPk : '',
                projectPk: values.projectPk  ? values.projectPk : '',
                tagPk: this.editTag.tagPk  ? this.editTag.tagPk : ''
              }
              putAction(this.url.edit, params).then(
                (res) => {
                  if (res.success) {
                    this.$message.success('修改成功');
                    this.$emit('getThingName',values.name, this.tagsId)
                    this.editTag = {}
                  } else {
                    this.$message.error(res.message);
                  }
