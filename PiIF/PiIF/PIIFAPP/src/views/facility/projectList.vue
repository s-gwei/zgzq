    ...
    <a-card :bordered="false" style="border-bottom: 1px solid #f1f1f1;">
      <a-row :gutter="48" type="flex" justify="start" style="margin-bottom: 20px;">
        <a-col :style="{borderRight: '1px solid #f1f1f1'}">
          <span :style="{fontSize: '24px', lineHeight: '40px', whiteSpace: 'nowrap'}">项目</span>
        </a-col>
        <a-col>
          <a-form layout="inline">
            <a-form-item label="">
              <a-input-search v-model="queryParam.projectName" @search="searchQuery" placeholder="输入项目名称查询"/>
            </a-form-item>
  ...
  export default {
    ...
    data () {
      return {
        ...
        url: {
          ...
          delete: '/iot/project/deleteProject',
          deleteBatch: '/iot/project/deleteProjectList'
        }
      }
    },
    created () {
    ...
    methods: {
      ...
      goDetail (num, record) {
        switch (num) {
          case 1:      // 添加逻辑
            this.$router.push({path: '/facility/projectDetail'})
            sessionStorage.setItem("projectInfo",JSON.stringify({tabKey: 1}));
            this.$bus.$emit('setCollapsed', true)
            break;
          case 2:      // 查看逻辑
            this.$router.push({path: '/facility/projectDetail'})
            sessionStorage.setItem("projectInfo",JSON.stringify({tabKey: 1,name: record.name,id: record.id,  type: 'file'}));
            this.$bus.$emit('setCollapsed', true)
            break;
          case 3:      // 编辑逻辑
            this.$router.push({path: '/facility/projectDetail'})
            sessionStorage.setItem("projectInfo",JSON.stringify({tabKey: 1,name: record.name,id: record.id,  type: 'edit'}));
            this.$bus.$emit('setCollapsed', true)
            break;
          case 4:      // 复制逻辑
            if (this.selectedRowKeys.length === 0) {
              ...
              this.$router.push({path: '/facility/projectDetail'})
              sessionStorage.setItem("projectInfo",JSON.stringify({tabKey: 1,name: this.selectedRows[0].name,id: this.selectedRows[0].id,  type: 'copy'}));
            }
            this.$bus.$emit('setCollapsed', true)
            break;
          case 5:      // 删除逻辑
      ...
      handleFile () {
        if (this.selectedRowKeys.length === 0) {
          ...
          this.$router.push({path: '/facility/projectDetail'})
          sessionStorage.setItem("projectInfo",JSON.stringify({tabKey: 1,name: this.selectedRows[0].name,id: this.selectedRows[0].id, type: 'file'}));
      ...
      handleEdit () {
        if (this.selectedRowKeys.length === 0) {
          ...
          this.$router.push({path: '/facility/projectDetail'})
          sessionStorage.setItem("projectInfo",JSON.stringify({tabKey: 1,name: this.selectedRows[0].name,id: this.selectedRows[0].id,  type: 'edit'}));
          this.$bus.$emit('setCollapsed', true)
        }
      },
      handleDel () {
