    ...
    <a-card :bordered="false" style="border-bottom: 1px solid #f1f1f1;">
      <a-row :gutter="48" type="flex" justify="start" style="margin-bottom: 20px;">
        <a-col :style="{borderRight: '1px solid #f1f1f1'}">
          <span :style="{fontSize: '24px', lineHeight: '40px', whiteSpace: 'nowrap'}">标签</span>
        </a-col>
        <a-col>
          <a-form layout="inline">
            <a-form-item label="">
              <a-input-search v-model="queryParam.tagName" @search="searchQuery" placeholder="输入标签名称查询"/>
            </a-form-item>
                ...
                <template slot="content">
                  <a-form style="width:400px;margin-top: 20px;">
                    <a-form-item label="标签:" :labelCol="labelCol" :wrapperCol="wrapperCol">
                      <a-select v-model="queryParam.tagPK" placeholder="请选择" :default-value="0">
                    ...
                    <a-form-item label="项目:" :labelCol="labelCol" :wrapperCol="wrapperCol">
                      <a-select v-model="queryParam.projectPk" placeholder="请选择" :default-value="0">
                    ...
                    <a-form-item style="overflow: hidden;border-top:1px solid #f1f1f1;padding-top:10px;margin-bottom: 0;">
                      <a-button type="primary" style="float:right;" @click="searchQuery">确认</a-button>
    ...
    <a-card :bordered="false">
      ...
      >
        <template slot="name" slot-scope="text, record">
          <a @click="goDetail(2, record)">{{text}}</a>
        </template>
...
<script>
  ...
  export default {
    ...
    data () {
      return {
        ...
        loadData: (parameter) => {
          ...
          return getAction(this.url.searchList, Object.assign(parameter ,params, this.queryParam)).then((res) => {
            if (res.success) {
              return {
        ...
        url: {
          ...
          searchList: '/iot/tag/query',
          projectList: 'iot/project/query',
          tags: '/iot/tag/getVocabularyByUserId',
          delete: '/iot/tag/deleteTag',
          deleteBatch: '/iot/tag/deleteTagList'
        }
      }
    },
    created () {
    ...
    methods: {
      ...
      // 查询项目
      getProjects () {
        getAction(this.url.list,{userId: this.userId, tableName: 'iot_project'}).then(
          (res) => {
            if (res.success) {
      ...
      goDetail (num, record) {
        switch (num) {
          case 1:      // 添加逻辑
            this.$router.push({path: '/facility/tagsDetail'})
            sessionStorage.setItem("tagsInfo",JSON.stringify({tabKey: 1}));
            this.$bus.$emit('setCollapsed', true)
            break;
          case 2:      // 查看逻辑
            this.$router.push({path: '/facility/tagsDetail'})
            sessionStorage.setItem("tagsInfo",JSON.stringify({tabKey: 1,name: record.name,id: record.id,  type: 'file'}));
            this.$bus.$emit('setCollapsed', true)
            break;
          case 3:      // 编辑逻辑
            this.$router.push({path: '/facility/tagsDetail'})
            sessionStorage.setItem("tagsInfo",JSON.stringify({tabKey: 1,name: record.name,id: record.id,  type: 'edit'}));
            this.$bus.$emit('setCollapsed', true)
            break;
          case 4:      // 复制逻辑
            if (this.selectedRowKeys.length === 0) {
              this.$message.warning('您未选中标签');
            } else if (this.selectedRowKeys.length >= 1) {
              this.$router.push({path: '/facility/tagsDetail'})
              sessionStorage.setItem("tagsInfo",JSON.stringify({tabKey: 1,name: this.selectedRows[0].name,id: this.selectedRows[0].id,  type: 'copy'}));
            }
            this.$bus.$emit('setCollapsed', true)
            break;
          case 5:      // 删除逻辑
      ...
      handleFile () {
        if (this.selectedRowKeys.length === 0) {
          ...
          this.$router.push({path: '/facility/tagsDetail'})
          sessionStorage.setItem("tagsInfo",JSON.stringify({tabKey: 1,name: this.selectedRows[0].name,id: this.selectedRows[0].id, type: 'file'}));
      ...
      handleEdit () {
        if (this.selectedRowKeys.length === 0) {
          ...
          this.$router.push({path: '/facility/tagsDetail'})
          sessionStorage.setItem("tagsInfo",JSON.stringify({tabKey: 1,name: this.selectedRows[0].name,id: this.selectedRows[0].id,  type: 'edit'}));
          this.$bus.$emit('setCollapsed', true)
        }
      },
      handleDel () {
