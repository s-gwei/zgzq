    ...
    <a-card :bordered="false" style="border-bottom: 1px solid #f1f1f1;">
      <a-row :gutter="48" type="flex" justify="start" style="margin-bottom: 20px;">
        <a-col :style="{ borderRight: '1px solid #f1f1f1'}">
          <span :style="{fontSize: '24px', lineHeight: '40px', whiteSpace: 'nowrap'}">设备实例</span>
        </a-col>
        <a-col >
          <a-form layout="inline">
            <a-form-item label="">
              <a-input-search v-model="queryParam.thingName" @search="editData" placeholder="输入实例名称查询"/>
            </a-form-item>
    ...
    methods: {
      ...
      goDetail (num, record) {
        switch (num) {
          case 1:      // 添加逻辑
            this.$router.push({path: '/facility/ModelDetail'})
            sessionStorage.setItem("ModelInfo",JSON.stringify({tabKey: 1,type:'facility'}));
            this.$bus.$emit('setCollapsed', true)
            break;
          case 2:      // 查看逻辑
            this.$router.push({path: '/facility/ModelDetail'})
            sessionStorage.setItem("ModelInfo",JSON.stringify({tabKey: 1, type:'facility',name: record.name,detail:true}));
            this.$bus.$emit('setCollapsed', true)
            break;
          case 3:      // 编辑逻辑
            this.$router.push({path: '/facility/ModelDetail'})
            sessionStorage.setItem("ModelInfo",JSON.stringify({tabKey: 1, type:'facility',name: record.name}));
            this.$bus.$emit('setCollapsed', true)
            break;
          case 4:      // 复制逻辑
            if (this.selectedRowKeys.length === 0) {
              ...
              this.$router.push({path: '/facility/ModelDetail'});
              sessionStorage.setItem("ModelInfo",JSON.stringify({tabKey: 1, type:'facility',name: this.selectedRows[0].name,copy:true}));
              this.$bus.$emit('setCollapsed', true)
          ...
          case 6:      // 权限逻辑
            if (this.selectedRowKeys.length === 0) {
              ...
              this.$router.push({path: '/facility/ModelDetail'})
              sessionStorage.setItem("ModelInfo",JSON.stringify({tabKey: 6, type:'facility',name: this.selectedRows[0].name}));
              this.$bus.$emit('setCollapsed', true)
            }
            break;
        }
      ...
      handleFile () {
        if (this.selectedRowKeys.length === 0) {
          ...
          this.$router.push({path: '/facility/ModelDetail'})
          sessionStorage.setItem("ModelInfo",JSON.stringify({tabKey: 1, type:'facility',name: this.selectedRows[0].name,detail: true}));
          this.$bus.$emit('setCollapsed', true)
        }
      },
...
//        this.mdl = Object.assign({}, record)
        if (this.selectedRowKeys.length === 0) {
          ...
          this.$router.push({path: '/facility/ModelDetail'})
          sessionStorage.setItem("ModelInfo",JSON.stringify({tabKey: 1, type:'facility',name: this.selectedRows[0].name}));
          this.$bus.$emit('setCollapsed', true)
        }
        this.$bus.$emit('setCollapsed', true)
      },
