...
public class SysUserController {
    ...
    public Result<List<SysUser>> queryPageList() {
    ...
    public Result<List<String>> queryRoleCode(@RequestParam(name = "userid", required = true) String userid) {
        Result<List<String>> result = new Result<>();
        SysUser user = sysUserService.getById(userid);
        //获取角色编码
        List<String> rolesByUsername = sysBaseAPI.getRolesByUsername(user.getUsername());
        if (rolesByUsername == null || rolesByUsername.size() <= 0) {
            result.error500("未找到用户相关角色信息");
        }

        result.setSuccess(true);
        result.setResult(rolesByUsername);
        return result;
    }
