        ...
        try {
            ...
            if (departs.size()>0){
                for (SysDepart depart : departs) {
                    List<SysRole> list=sysRoleService.getRoleByDepart(depart);
            ...
            metaList.addAll(perList_1);
            metaList.addAll(perList_2);
            //权限去重
            metaList=init(metaList);
            //添加首页路由
